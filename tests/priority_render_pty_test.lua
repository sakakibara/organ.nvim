-- Real-redraw guard for priority cookies: the rank letter must ACTUALLY
-- render at the window's right edge, and the raw `[#A]` must be concealed
-- from the left content. Placing an extmark is not the same as rendering it
-- (nvim's ephemeral provider drops inline virt_text); this runs nvim under a
-- real pty, redraws, and reads the rendered cells via screenstring().
--
-- Skips gracefully when a usable pty `script` is unavailable.
--
-- Run via: nvim --headless -l tests/priority_render_pty_test.lua
...
local function skip(why)
  print("(skipped: " .. why .. ")")
  print("priority_render_pty_test: SKIP")
  os.exit(0)
end
...
vim.api.nvim_buf_set_lines(b, 0, -1, false, { "* [#A] ship it" })
vim.bo[b].filetype = "org"
vim.wo.conceallevel = 2
vim.wo.concealcursor = "nc"
pcall(vim.treesitter.start, b, "org")
require("organ.modern.priority").attach(b)
vim.cmd("redraw")
local right = ""
for c = 33, 40 do right = right .. vim.fn.screenstring(1, c) end
local left = ""
for c = 1, 20 do left = left .. vim.fn.screenstring(1, c) end
...
check("rank letter A renders near the right edge", kv.right and kv.right:find("A") ~= nil,
  "right dump: " .. tostring(kv.right))
check("the raw [#A] is concealed from the left content", kv.left and kv.left:find("%[#A%]") == nil,
...
print()
print("priority_render_pty_test: PASS")
os.exit(0)
