...
check(
  "returns body + cap group names",
  body_hl == "@organ.modern.badge.t1" and cap_hl == "@organ.modern.badgecap.t1"
)
...
for _, m in ipairs(marks) do
  local d = m[4]
  if d.hl_group == body_hl then
    body = m
  elseif d.virt_text and d.virt_text_pos == "inline" and m[3] == 2 then
    left = d
  elseif d.virt_text and d.virt_text_pos == "inline" and m[3] == 6 then
    right = d
  end
end
check("body extmark spans the range", body ~= nil and body[3] == 2 and body[4].end_col == 6)
check(
  "left cap is INLINE at sc (preserves the space before)",
  left ~= nil,
  "no inline cap at col 2"
)
check(
  "right cap is INLINE at ec (preserves the space after)",
  right ~= nil,
