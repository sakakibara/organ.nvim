-- Real-redraw guard for plain list bullets: `-` must ACTUALLY render as `•`
-- at the bullet column, with the raw `-` concealed. Runs nvim under a real
-- pty, redraws, and reads the rendered cells via screenstring().
--
-- Skips gracefully when a usable pty `script` is unavailable.
--
-- Run via: nvim --headless -l tests/list_bullets_render_pty_test.lua
...
local function skip(why)
  print("(skipped: " .. why .. ")")
  print("list_bullets_render_pty_test: SKIP")
  os.exit(0)
end
...
vim.api.nvim_buf_set_lines(b, 0, -1, false, { "- item" })
vim.bo[b].filetype = "org"
vim.wo.conceallevel = 2
vim.wo.concealcursor = "nc"
pcall(vim.treesitter.start, b, "org")
require("organ.modern.list_bullets").attach(b)
vim.cmd("redraw")
local bullet = vim.fn.nr2char(0x2022)
local found = "nil"
local cells = {}
for c = 1, 20 do
  local s = vim.fn.screenstring(1, c)
  cells[#cells + 1] = (s == "" and "_" or s)
  if s == bullet and found == "nil" then found = tostring(c) end
end
...
print()
print("list_bullets_render_pty_test: PASS")
os.exit(0)
