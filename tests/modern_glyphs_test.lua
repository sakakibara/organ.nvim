...
check(
  "nerd mode returns the nerd cap",
  glyphs.get("pill.cap.left", b) == glyphs._GLYPHS["pill.cap.left"].nerd
)
require("organ.buf_config").set(b, "modern.nerd_font", false)
check(
  "ascii mode returns the ascii cap",
  glyphs.get("pill.cap.left", b) == glyphs._GLYPHS["pill.cap.left"].ascii
)
