...
function M.attach(bufnr)
  ...
  if bc.read(bufnr, "modern.directives") then
    require("organ.modern.directives").attach(bufnr)
  end
  if bc.read(bufnr, "modern.drawers") then
    require("organ.modern.drawers").attach(bufnr)
  end
...
function M.detach(bufnr)
  bufnr = bufnr or vim.api.nvim_get_current_buf()
  for _, sub in ipairs({ "bullets", "blocks", "pills", "priority", "tags", "cookies", "checkboxes", "list_bullets", "dates", "rule", "directives", "drawers", "table" }) do
    pcall(function()
